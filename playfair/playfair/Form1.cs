...
namespace playfair
{
    public partial class Form1 : Form
    {
        ...
        {
            // Lưu vị trí và ký tự không hợp lệ
            var nonLetters = ciphertext
                .Select((c, i) => new { Char = c, Index = i })
                .Where(x => !char.IsLetter(x.Char))
                .ToList();
            ...
            // Chèn lại các ký tự không hợp lệ vào vị trí cũ
            foreach (var nonLetter in nonLetters)
            {
                plaintext.Insert(nonLetter.Index, nonLetter.Char);
            }
